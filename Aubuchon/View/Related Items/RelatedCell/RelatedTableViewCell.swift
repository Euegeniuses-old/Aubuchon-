    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
       ...
       cell.productName.text = productDisplayData[indexPath.row].webDesc
        ...
        if productDisplayData[indexPath.row].image == "" {
             cell.productImage.image = UIImage(named: "AppIcon")
            
        } else {
             cell.productImage.sd_setImage(with: URL(string: productDisplayData[indexPath.row].image), placeholderImage: UIImage(named: "AppIcon"))
        }
        return cell
    }
